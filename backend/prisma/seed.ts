...
async function main() {
  ...
  try {
    ...
    const adminUser = await prisma.users.upsert({
      where: { email: 'tkgmfb01@gmail.com' },
      update: {
        username: 'admin',
    ...
    const categories = [
      ...
      },
      {
        name: 'Hỏi đáp',
        slug: 'hoi-dap',
        description: 'Bài đăng nhằm mục đích đặt câu hỏi, giải đáp thắc mắc',
        color: '#3498DB',
        sort_order: 6,
        view_permission: PermissionLevel.ALL,
        post_permission: PermissionLevel.MEMBER,
      ...
      },
      {
        name: 'Off-topic',
        slug: 'off-topic',
        description: 'Thảo luận về các chủ đề ngoài lề. Tập trung những thứ khó phân loại',
        color: '#95A5A6',
        sort_order: 7,
        view_permission: PermissionLevel.ALL,
        post_permission: PermissionLevel.MEMBER,
