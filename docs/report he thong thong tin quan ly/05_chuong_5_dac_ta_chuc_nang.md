...
## 5.0 Tổng quan 10 module chức năng

MINI-FORUM tổ chức thành 10 module độc lập, mỗi module bao gồm controller, service, routes. Tổng cộng **~70 API endpoints** RESTful tuân thủ chuẩn: phương thức HTTP, đường dẫn rõ ràng, xác thực/phân quyền thống nhất tại middleware layer. Mỗi module được mô tả bao gồm: cấu trúc code, danh sách API endpoints (HTTP method, path, auth requirement, rate limiting), business rule trọng yếu, quyết định thiết kế kỹ thuật.
...
Module xác thực là nền tảng bảo mật. Mọi request đến endpoint được bảo vệ phải đi qua `authMiddleware.authenticate()` — kiểm tra JWT, tải user từ DB, gắn vào `req.user`. Ba nguyên tắc: (1) mật khẩu lưu hash bcrypt không plain text, (2) access token thời gian sống ngắn, (3) refresh token có thể revoke ngay lập tức.

Middleware pipeline: request → helmet (security headers) → cors → rate-limit → authMiddleware (verify JWT + load user) → roleMiddleware (RBAC check) → controller. Tổng 8 endpoints: `/auth/register`, `/auth/verify-otp`, `/auth/login`, `/auth/refresh`, `/auth/logout`, `/auth/forgot-password`, `/auth/reset-password`, `/auth/resend-otp`.
