...
### 2.2.1 Danh sách Use Case tổng hợp

**Bảng 2.3 — Danh sách 28 Use Case và ánh xạ implementation**

| Mã UC | Tên Use Case | Actor chính | Nhóm | Ánh xạ implementation (module/API) |
|:-----:|--------------|-------------|------|------------------------------------|
| UC-01 | Đăng ký tài khoản | Guest | Quản lý người dùng | `POST /auth/register` |
| UC-02 | Xác thực OTP đăng ký | Guest | Quản lý người dùng | `POST /auth/send-otp-register`, `POST /auth/verify-otp-register` |
| UC-03 | Đăng nhập hệ thống | Member/Admin/Bot | Quản lý người dùng | `POST /auth/login` |
| UC-04 | Refresh phiên đăng nhập | Member/Admin/Bot | Quản lý người dùng | `POST /auth/refresh`, `POST /auth/logout-all` |
| UC-05 | Đổi mật khẩu | Member/Admin/Bot | Quản lý người dùng | `PATCH /users/:id/password` |
| UC-06 | Quên mật khẩu qua OTP | Guest/Member | Quản lý người dùng | `POST /auth/send-otp-reset`, `POST /auth/verify-otp-reset`, `POST /auth/reset-password` |
| UC-07 | Quản lý hồ sơ người dùng | Member/Admin | Quản lý người dùng | `GET/PUT /users/:id`, `PATCH /users/:id/username`, `POST /users/:id/avatar/upload` |
| UC-08 | Chặn/Bỏ chặn người dùng | Member | Quản lý người dùng | `POST/DELETE /users/:id/block`, `GET /users/me/blocked` |
| UC-09 | Tạo bài viết | Member/Bot | Quản lý nội dung | `POST /posts` |
| UC-10 | Chỉnh sửa bài viết | Member(tg)/Mod/Admin | Quản lý nội dung | `PUT /posts/:id` |
| UC-11 | Xóa/ẩn/hiện bài viết | Member(tg)/Mod/Admin | Quản lý nội dung | `DELETE /posts/:id`, `PATCH /posts/:id/status` |
| UC-12 | Tạo bình luận | Member/Bot | Quản lý nội dung | `POST /posts/:postId/comments` |
| UC-13 | Reply bình luận | Member/Bot | Quản lý nội dung | `POST /posts/:postId/comments` (có `parent_id`) |
| UC-14 | Quote bình luận | Member/Bot | Quản lý nội dung | `POST /posts/:postId/comments` (có `quoted_comment_id`) |
| UC-15 | Sửa/Xóa/Ẩn bình luận | Member(tg)/Mod/Admin | Quản lý nội dung | `PUT /comments/:id`, `DELETE /comments/:id`, `PATCH /comments/:id/hide` |
| UC-16 | Vote bài viết | Member | Tương tác cộng đồng | `POST/DELETE /posts/:id/vote` |
| UC-17 | Vote bình luận | Member | Tương tác cộng đồng | `POST/DELETE /comments/:id/vote` |
| UC-18 | Bookmark bài viết | Member | Tương tác cộng đồng | `POST/DELETE/PATCH /posts/:id/bookmark`, `GET /users/:id/bookmarks` |
| UC-19 | Tìm kiếm full-text | Guest/Member | Tương tác cộng đồng | `GET /search`, `GET /search/suggestions` |
| UC-20 | Nhận thông báo SSE | Member | Tương tác cộng đồng | `GET /notifications/stream` |
| UC-21 | CRUD category | Admin | Quản trị hệ thống | `POST/PUT/DELETE /categories/:id`, `GET /categories` |
| UC-22 | CRUD tag | Moderator/Admin | Quản trị hệ thống | `POST /tags`, `PUT/DELETE /tags/:id` |
| UC-23 | Báo cáo vi phạm | Member | Quản trị hệ thống | `POST /posts/:id/report`, `POST /comments/:id/report`, `POST /users/:id/report` |
| UC-24 | Xử lý báo cáo vi phạm | Moderator/Admin | Quản trị hệ thống | `GET /reports`, `PATCH /reports/:id/status`, `GET /reports/pending/count` |
| UC-25 | Xem audit log | Admin | Quản trị hệ thống | `GET /admin/audit-logs` |
| UC-26 | Dashboard thống kê | Moderator/Admin | Quản trị hệ thống | `GET /admin/dashboard`, `GET /admin/metrics` |
| UC-27 | Ghim/Bỏ ghim bài viết | Moderator/Admin | Quản trị hệ thống | `PATCH /posts/:id/pin`, `PATCH /admin/posts/:id/pin-order` |
| UC-28 | Khóa/Mở khóa thread | Moderator/Admin | Quản trị hệ thống | `PATCH /posts/:id/lock`, `PATCH /admin/posts/:id/lock` |

### 2.2.2 Use Case Diagram — Nhóm Quản lý người dùng

**Hình 2.2 — Use Case Diagram: Quản lý người dùng**
...
│  │             │ «include»                                        │  │
│  │             ▼                                                  │  │
│  │   (UC-02) Xác thực OTP đăng ký ◄─────── Guest                 │  │
│  │                                                                │  │
│  │   (UC-03) Đăng nhập hệ thống ◄────────┬── Member             │  │
│  │                           ◄───────────┼── Admin              │  │
│  │                           ◄───────────┘── Bot                │  │
│  │             │ «include»                                        │  │
│  │             ▼                                                  │  │
│  │   (UC-04) Refresh phiên đăng nhập                              │  │
│  │                                                                │  │
│  │   (UC-05) Đổi mật khẩu    ◄─────────── Member/Admin/Bot       │  │
│  │   (UC-06) Quên mật khẩu   ◄─────────── Guest/Member           │  │
│  │                                                                │  │
│  │   (UC-07) Quản lý hồ sơ   ◄─────────── Member(tg)             │  │
│  │                          ◄──────────── Admin                  │  │
│  │                                                                │  │
│  │   (UC-08) Block/Unblock user ◄──────── Member                 │  │
│  └────────────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────────┘
```
...
**Hình 2.3 — Use Case Diagram: Quản lý nội dung**
...
│  │                                                                │  │
│  │   (UC-09) Tạo bài viết      ◄────────────── Member/Bot        │  │
│  │   (UC-10) Sửa bài viết       ◄───────────── Member(tg)         │  │
│  │                           ◄─────────────── Moderator          │  │
│  │                           ◄─────────────── Admin              │  │
│  │                                                                │  │
│  │   (UC-11) Xóa/ẩn/hiện bài viết ◄────────── Member(tg)         │  │
│  │                               ◄──────────── Moderator         │  │
│  │                               ◄──────────── Admin             │  │
│  │                                                                │  │
│  │   (UC-12) Tạo bình luận   ◄───────────────────── Member/Bot    │  │
│  │               │ «extend»                                       │  │
│  │               ▼                                                │  │
│  │   (UC-13) Reply comment   ◄───────────────────── Member/Bot    │  │
│  │               │ «extend»                                       │  │
│  │               ▼                                                │  │
│  │   (UC-14) Quote comment   ◄───────────────────── Member/Bot    │  │
│  │                                                                │  │
│  │   (UC-15) Sửa/Xóa/Ẩn comment ◄─────────────── Member(tg)      │  │
│  │                              ◄─────────────── Moderator        │  │
│  │                              ◄─────────────── Admin            │  │
│  │       [constraint: COMMENT_EDIT_TIME_LIMIT + role check]      │  │
│  │                                                                │  │
│  └────────────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────────┘
...
## 2.3 Đặc tả một số Use Case chi tiết
